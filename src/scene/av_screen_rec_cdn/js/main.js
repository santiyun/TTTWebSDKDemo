...
function createPublishAudioVideoStream()
{
	...
	if (tttStatus !== 1)
	{
		console.log('<demo> createPublishAudioVideoStream - 请先[加入房间]');
		return;
	}

	// for Audio stream
	gAudioVideoStream = window.RTCObj.createStream({
		userid: +userId,
		audio: true,
		video: true
	});
...
function publishAudioVideoStream()
{
	...
	if (tttStatus !== 1)
	{
		console.log('<demo> publishAudioVideoStream - 请先[加入房间]');
		return;
	}
...
function publishScreenStream()
{
	...
	if (tttStatus !== 1)
	{
		console.log('<demo> publishScreenStream - 请先[加入房间]');
		return;
	}
